...
@implementation UIImage (EjectAlertView)
...
@interface EjectAlertScheduler : NSObject
...
@interface EjectAlertLayer: CALayer
...
@implementation EjectAlertLayer
...
@implementation EjectAlertScheduler

+ (instancetype)sharedInstance
{
    static EjectAlertScheduler *sharedInstance = nil;
    if (!sharedInstance)
    {
        sharedInstance = [[EjectAlertScheduler alloc] init];
    }
...
- (void)setBlurEnabled:(BOOL)blurEnabled
{
    ...
    if (blurEnabled)
    {
        for (EjectAlertView *view in self.views)
        {
...
- (void)addView:(EjectAlertView *)view
{
...
- (void)removeView:(EjectAlertView *)view
{
...
- (void)updateAsynchronously
{
    if (self.blurEnabled && !self.updating && self.updatesEnabled > 0 && [self.views count])
    {
        ...
        {
            EjectAlertView *view = self.views[i];
...
@implementation EjectAlertView

+ (void)setBlurEnabled:(BOOL)blurEnabled
{
    [EjectAlertScheduler sharedInstance].blurEnabled = blurEnabled;
}

+ (void)setUpdatesEnabled
{
    [[EjectAlertScheduler sharedInstance] setUpdatesEnabled];
}

+ (void)setUpdatesDisabled
{
    [[EjectAlertScheduler sharedInstance] setUpdatesDisabled];
}

+ (Class)layerClass
{
    return [EjectAlertLayer class];
}

- (void)setUp
{
    if (!_iterationsSet) _iterations = 3;
    if (!_blurRadiusSet) [self blurLayer].blurRadius = 1;
...
- (EjectAlertLayer *)blurLayer
{
    return (EjectAlertLayer *)self.layer;
}

- (EjectAlertLayer *)blurPresentationLayer
{
    EjectAlertLayer *blurLayer = [self blurLayer];
    return (EjectAlertLayer *)blurLayer.presentationLayer ?: blurLayer;
}
...
- (void)schedule
{
    ...
    {
        [[EjectAlertScheduler sharedInstance] addView:self];
    }
    else
    {
        [[EjectAlertScheduler sharedInstance] removeView:self];
    }
}
...
- (BOOL)shouldUpdate
{
    ...
    self.blurEnabled && [EjectAlertScheduler sharedInstance].blurEnabled &&
...
- (UIImage *)snapshotOfUnderlyingView
{
    __strong EjectAlertLayer *blurLayer = [self blurPresentationLayer];
    __strong CALayer *underlyingLayer = [self underlyingLayer];
