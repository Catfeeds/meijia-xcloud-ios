...
//        headIamageView.image=[UIImage imageWithData:[NSData dataWithContentsOfURL:[NSURL URLWithString:[dict objectForKey:@"head_img"]]]];
    ...
    if(_rootId==1){
        tableScrollView=[[UITableView alloc]initWithFrame:FRAME(0, blurView.frame.size.height+blurView.frame.origin.y+20, WIDTH, HEIGHT-blurView.frame.size.height-114) style:UITableViewStyleGrouped];
    }else{
        tableScrollView=[[UITableView alloc]initWithFrame:FRAME(0, blurView.frame.size.height+blurView.frame.origin.y, WIDTH, HEIGHT-blurView.frame.size.height-50) style:UITableViewStyleGrouped];
    }
    
    //设置数据
    tableDataArr = [NSMutableArray array];
    NSArray * firstImageArr = @[@"myself_youhuiquan",@"myself_dingdan",@"myself_jinbihuangou",@"myself_huiyuanjulebu"];
    NSArray * firstTitleArr = @[@"优惠券",@"我的订单",@"金币换购",@"会员俱乐部"];
    NSArray * firstTextArr  = @[@"",@"",@"0元好物在这里",@"沙龙活动限时报名"];
    NSMutableArray * firstDataArr = [NSMutableArray array];
    
    NSArray * secondImageArr = @[@"myself_haopingsongli",@"myself_tuijianhaoyou",@"myself_jiamenghezuo",@"myself_gengduo"];
    NSArray * secondTitleArr = @[@"好评送好礼",@"推荐给好友",@"加盟合作入口",@"更多"];
    NSArray * secondTextArr  = @[@"好评截图有好礼",@"流量送不停",@"诚邀入驻",@"新版本记得更新哦"];
    NSMutableArray * secondDataArr = [NSMutableArray array];
    for (NSInteger i = 0; i < firstImageArr.count; i++) {
        NSDictionary * firstDict = @{@"image":firstImageArr[i],@"title":firstTitleArr[i],@"text":firstTextArr[i]};
        [firstDataArr addObject:firstDict];
        
        NSDictionary * secondDict = @{@"image":secondImageArr[i],@"title":secondTitleArr[i],@"text":secondTextArr[i]};
        [secondDataArr addObject:secondDict];
    }
    [tableDataArr addObject:firstDataArr];
    [tableDataArr addObject:secondDataArr];
    

    tableScrollView.backgroundColor=[UIColor colorForHex:@"#f5f5f5"];
    [tableScrollView  setSeparatorColor :[UIColor colorForHex:@"eeeeee"]];
  
//    tableScrollView.contentSize=CGSizeMake(WIDTH, 131*3+80);
    
    tableScrollView.delegate=self;
    tableScrollView.dataSource = self;
...
//    NSString *se=@"a8c44753ced7bac4f98d6611c4db1901";
    switch (tg) {
        case 100:
        {
            /*]WithFetchRequest:[[UMComUserFeedsRequest alloc] initWithUid:_manager.telephone count:BatchSize type:UMComTimeLineTypeDefault]];*/
...
{
    NSString *moneyString=[NSString stringWithFormat:@"%@",[dict objectForKey:@"rest_money"]];
    NSLog(@"%@",moneyString);
    if (moneyString==NULL||moneyString==nil||[moneyString length]==0) {
        cradString=@"0元";
    }else{
        cradString=[NSString stringWithFormat:@"%@元",moneyString];
    }
...
-(NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    NSArray * sectionDataArr = tableDataArr[section];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    UITableViewCell * cell = [tableView dequeueReusableCellWithIdentifier:@"msMyselfTable"];
    if (cell == nil) {
        cell = [[UITableViewCell alloc]initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:@"msMyselfTable"];
    }
    NSDictionary * dataDict = tableDataArr[indexPath.section][indexPath.row];
    cell.imageView.image = [UIImage imageNamed:dataDict[@"image"]];
    cell.textLabel.text = dataDict[@"title"];
    cell.textLabel.textColor = [UIColor colorForHex:@"333333"];
    cell.textLabel.font =[UIFont systemFontOfSize:16];
    
    cell.detailTextLabel.text = dataDict[@"text"];
    cell.detailTextLabel.font = [UIFont systemFontOfSize:12];
    cell.detailTextLabel.textColor = [UIColor colorForHex:@"9a9a9a"];
    ...
    if (indexPath.section == 0) {
        switch (indexPath.row) {
            ...
            case 3:
            {
                //会员俱乐部
                ISLoginManager *_manager = [ISLoginManager shareManager];
                NSString *url=[NSString stringWithFormat:@"http://app.bolohr.com/simi-h5/show/vipclub-index.html?userid=%@",_manager.telephone];
                CreditWebViewController *web=[[CreditWebViewController alloc]initWithUrl:url];//实际中需要改为带签名的地址
                //如果已经有UINavigationContoller了，就 创建出一个 CreditWebViewController 然后 push 进去
                [self.navigationController pushViewController:web animated:YES];
            }
        ...
        switch (indexPath.row) {
            ...
            case 2:
            {
                ...
                NSString *url=[NSString stringWithFormat:@"http://app.bolohr.com/simi-h5/show/biz-list.html?userid=%@",_manager.telephone];
                CreditWebViewController *web=[[CreditWebViewController alloc]initWithUrl:url];//实际中需要改为带签名的地址
                //如果已经有UINavigationContoller了，就 创建出一个 CreditWebViewController 然后 push 进去
                [self.navigationController pushViewController:web animated:YES];
            }
