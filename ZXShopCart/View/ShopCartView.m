...
- (void)layoutUI {
    ...
    [self.payButton setTitle:[NSString stringWithFormat:@"选择领用品"] forState:UIControlStateNormal];
    ...
    // 背景视图
    self.popOutView = [[PopOutView alloc] initWithFrame:CGRectMake(0, 64, kWindowWidth, HEIGHT-50- 64)];
...
// 点击结算按钮
- (void)goToCheck {
    [[NSNotificationCenter defaultCenter] postNotificationName:@"DISTRICT" object:_orderArray];
...
- (void)setTotalPrice:(double)inTotal {
    ...
    if(self.inTotal > 0) {
        ...
        self.totalPriceLabel.text = [NSString stringWithFormat:@"共:%ld件",(long)_badgeValue+1];
        
        self.payButton.enabled = YES;
        [self.payButton setTitle:@"去领用" forState:UIControlStateNormal];
        [self.payButton setBackgroundColor:HEX_TO_UICOLOR(0x11cd6e, 1.0)];
        [self.shopCartBtn setUserInteractionEnabled:YES];
    } else {
        [self.totalPriceLabel setTextColor:[UIColor grayColor]];
        [self.totalPriceLabel setText:@"购物车空空如也~"];
...
- (void)updateFrame:(UIView *)view {
    ...
    self.detailListView.frame = CGRectMake(self.detailListView.frame.origin.x, self.parentView.bounds.size.height - height - 50-64, self.detailListView.frame.size.width, height);
