...
static CGFloat const imageBGHeight = 395 + 57 - 32; // 背景图片的高度
...
//  arraY=@[@"精选",@"职场",@"案例",@"招聘",@"薪资",@"行政",@"培训",@"绩效",@"员工关系",@"人资规划",@"行业"];
    arraY=@[@"头条",@"招聘",@"绩效",@"薪资",@"职场",@"案例",@"行政",@"培训",@"员工关系",@"人资规划",@"行业"];
    ...
    //需求4的话，就是更改headerView的高度
    //加的这个47就是新的UI多出来的高度。
    headeView=[[UIView alloc]initWithFrame:FRAME(0, 0, WIDTH, 395 + 57 - 32)];
...
//    NSArray *butArray=@[@"内推悬赏",@"同行热聊",@"工具资料",@"找服务商"/*@"知识学院",@"签到有礼",@"简历交换",@"找服务商",@"福利商城"*/];
    ...
    for (int i=0; i<butArray.count; i++) {
        if (i<4) {
            UIButton *button=[[UIButton alloc]initWithFrame:FRAME((WIDTH-200)/5+(50+(WIDTH-200)/5)*i, 168+35/2, 50, 50)];
...
//            butNameLabel.backgroundColor=[UIColor redColor];
            ...
            UIButton *button=[[UIButton alloc]initWithFrame:FRAME((WIDTH-160)/4/2+(40+(WIDTH-160)/4)*x, 256, 40, 40)];
            [button setImage:[UIImage imageNamed:butImageArray[i]] forState:UIControlStateNormal];
            button.tag=i;
            [button addTarget:self action:@selector(ButAction:) forControlEvents:UIControlEventTouchUpInside];
            button.layer.cornerRadius=20;
    ...
    UIView *lineViews=[[UIView alloc]initWithFrame:FRAME(0, 305 - 32, WIDTH, 10)];
    ...
    //添加左边的按钮,也就是HR圈内招聘
    //暂不添加点击方法，等确认跳转位置后添加
    UIButton * headLiftBut = [[UIButton alloc]initWithFrame:FRAME(0, 315 - 32, WIDTH / 2 - 0.5, 126)];
    [headeView addSubview:headLiftBut];
    //添加左边按钮标题
    UILabel * leftTitle = [[UILabel alloc]initWithFrame:FRAME((WIDTH /2  - 133) /3, 20, 83, 20)];
    leftTitle.font = [UIFont systemFontOfSize:14];
    leftTitle.textAlignment = NSTextAlignmentCenter;
    
    NSMutableAttributedString *AttributedStr = [[NSMutableAttributedString alloc]initWithString:@"HR 圈内招聘"];
    ...
    leftTitle.attributedText = AttributedStr;
    [headLiftBut addSubview:leftTitle];
    //添加左边按钮描述
    UILabel * leftText = [[UILabel alloc]initWithFrame:FRAME((WIDTH / 2- 133) /3, 50, 79, 25)];
    leftText.numberOfLines = 2;
    leftText.textColor = [UIColor colorForHex:@"888888"];
    leftText.font = [UIFont systemFontOfSize:10];
    leftText.textAlignment = NSTextAlignmentCenter;
    leftText.text = @"圈内推荐更靠谱同行内推赚佣金";
    [headLiftBut addSubview:leftText];
    //添加左边的按钮图片
    UIImageView * leftImage = [[UIImageView alloc]initWithFrame:FRAME(WIDTH / 2 - (WIDTH / 2 - 133) /3 - 53, 30, 53, 55)];
    leftImage.image = [UIImage imageNamed:@"homePage_zhaopin"];
    [headLiftBut addSubview:leftImage];
    
    //添加一个背景view
    UIView * backGroundView = [[UIView alloc]initWithFrame:FRAME((WIDTH / 2 - 133) /3 + 8, 85, 60, 17)];
    backGroundView.backgroundColor =[UIColor colorForHex:@"#fd6663"];
    backGroundView.layer.cornerRadius = 7;
    backGroundView.layer.masksToBounds = YES;
    [headLiftBut addSubview:backGroundView];
    
    //添加左边下面的text
    UILabel * leftViewText = [[UILabel alloc]initWithFrame:FRAME((WIDTH / 2 - 133) /3 + 8, 84, 60, 17)];

    NSMutableAttributedString *leftViewTextAttributedStr = [[NSMutableAttributedString alloc]initWithString:@" 去看看 ▸"];
    [leftViewTextAttributedStr addAttribute:NSFontAttributeName
    ...
    leftViewText.attributedText = leftViewTextAttributedStr;
    leftViewText.textColor =[UIColor colorForHex:@"ffffff"];
//    leftViewText.layer.cornerRadius = 7;
//    leftViewText.layer.masksToBounds = YES;
    leftViewText.textAlignment = NSTextAlignmentCenter;
//    leftViewText.backgroundColor = [UIColor colorForHex:@"#fd6663"];
    [headLiftBut addSubview:leftViewText];
    ...
    [headLiftBut setBackgroundImage:[self imageWithColor:[UIColor lightGrayColor]] forState:UIControlStateHighlighted];
    //添加分割线
    UIView *verticalView=[[UIView alloc]initWithFrame:FRAME(WIDTH/2-0.5, 315 - 32, 0.5, 126)];
    ...
    //定义右边的两个按钮。
    NSArray * titleArr = @[@"签到 有好礼",@"考证 看这里"];
    NSArray * textArr = @[@"每日签到 满满收获",@"模拟试题 考试技巧"];
    NSArray * imageArr = @[@"homePage_qiandao",@"homePage_kaozheng"];
    
    
    
    for (NSInteger i = 0; i < 2; i++) {
        //按钮
        UIButton * rightButton = [[UIButton alloc]initWithFrame:FRAME(WIDTH/2+0.5, 315 + i * 63 - 32, WIDTH/2-0.5, 63)];
        [headeView addSubview:rightButton];
        //标签
        UILabel * rightTitle = [[UILabel alloc]initWithFrame:FRAME(13, 10, 80, 20)];
        NSMutableAttributedString *rightTitleStr = [[NSMutableAttributedString alloc]initWithString:titleArr[i]];
        [rightTitleStr addAttribute:NSForegroundColorAttributeName
         
                              value:[UIColor redColor]
         
                              range:NSMakeRange(0, 2)];
        
        rightTitle.attributedText = rightTitleStr;
        ...
        //描述
        UILabel * rightText = [[UILabel alloc]initWithFrame:FRAME(13, 35, 85, 20)];
        rightText.text = textArr[i];
        rightText.textAlignment = NSTextAlignmentCenter;
        rightText.textColor = [UIColor colorForHex:@"888888"];
        rightText.font = [UIFont systemFontOfSize:10];
        [rightButton addSubview:rightText];
        //图片
        UIImageView * rightImage = [[UIImageView alloc]initWithFrame:FRAME(WIDTH / 2 - (WIDTH / 2 - 134) / 2 - 36, 15, 36, 32)];
        rightImage.image =[UIImage imageNamed:imageArr[i]];
        ...
        [rightButton addTarget:self action:@selector(ButAction:) forControlEvents:UIControlEventTouchUpInside];
        if (i == 0) {
            //添加分割线
            UIView *transverseView=[[UIView alloc]initWithFrame:FRAME(0, 62.5, WIDTH / 2, 0.5)];
            transverseView.backgroundColor=[UIColor colorWithRed:220/255.0f green:220/255.0f blue:220/255.0f alpha:1];
            [rightButton addSubview:transverseView];
        }
    }
    //以后需要注释掉，重新布局
...
//    searchLabel.textAlignment=NSTextAlignmentCenter;
    ...
    //需要更改imageBGHeight的值，然后上面的view的高就会变大，然后布局
...
//            [self presentViewController:communityViewController animated:YES completion:nil];
           //新版本
            if(fatherVc.loginYesOrNo){
                ...
                //新版本，跳转H5界面
                ISLoginManager *_manager = [ISLoginManager shareManager];
            NSString *url=[NSString stringWithFormat:@"http://app.bolohr.com/simi-h5/show/job-reward-list.html?user_id=%@",_manager.telephone];
...
//                [self.navigationController pushViewController:web animated:YES];
        ...
        case 5:
        {
            if(fatherVc.loginYesOrNo){
                ...
                //新版本，跳转H5界面
                ISLoginManager *_manager = [ISLoginManager shareManager];
                NSString *url=[NSString stringWithFormat:@"http://app.bolohr.com/simi-h5/show/sign-today.html?user_id=%@",_manager.telephone];
                WebPageViewController *webPageVC=[[WebPageViewController alloc]init];
                webPageVC.barIDS=100;
                webPageVC.qdIDl=1000;
                webPageVC.webURL=url;
        ...
        case 6:
        {
            ...
            NSString *url=[NSString stringWithFormat:@"http://bolohr.com/hrexam"];
            WebPageViewController *webPageVC=[[WebPageViewController alloc]init];
            webPageVC.barIDS=100;
            webPageVC.qdIDl=1000;
            webPageVC.webURL=url;
...
-(void)scrollviewLayout
{
    ...
    for (int i=0; i<imageArray.count; i++) {
        ...
        UIButton *viewImage=[[UIButton alloc]initWithFrame:FRAME(WIDTH*i, 0, WIDTH, 168)];
        viewImage.backgroundColor=[UIColor whiteColor];
        viewImage.tag=i;
        [viewsArray addObject:viewImage];
        UIImageView *imageView=[[UIImageView alloc]initWithFrame:FRAME(0, 0, WIDTH, 168)];
    ...
    if (adView==nil) {
        adView= [[CycleScrollView alloc]initWithFrame:FRAME(0, 0, WIDTH, 168) animationDuration:5];
...
{
    page=1;
    if (myTableView.contentOffset.y > 356) {
        NSIndexPath * index = [NSIndexPath indexPathForRow:0 inSection:0];
        CGRect cellView = [myTableView rectForRowAtIndexPath:index];
        CGRect rect = [myTableView convertRect:cellView toView:myTableView];
        CGPoint cellPoint = CGPointMake(0, rect.origin.y - 100);
        [myTableView setContentOffset:cellPoint animated:YES];
    }
