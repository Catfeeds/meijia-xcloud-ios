...
{
    _titleStr=titleStr;
    KZLinkLabel *label=(KZLinkLabel *)[self viewWithTag:104];
    label.text=titleStr;
    ...
    label.linkTapHandler = ^(KZLinkType linkType, NSString *string, NSRange range){
        if (linkType == KZLinkTypeURL) {
            self.seletedUrlBlock(string);
        }
    };
...
//    
//    NSArray *resultArray = [expression matchesInString:label.text options:0 range:NSMakeRange(0,titleStr.length)];
//    
//
