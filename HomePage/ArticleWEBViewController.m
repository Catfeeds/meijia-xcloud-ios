...
-(void) loadMyWebView{
    ...
    //正式接口
    NSString *urlString = [NSString stringWithFormat:@"http://bolohr.com/?json=get_post&post_id=%@&include=id,title,modified,url,thumbnail,custom_fields,content,categories",_listID];
//    NSLog(@"%@",urlString);
    ...
    [mymanager GET:[NSString stringWithFormat:@"%@",urlString] parameters:nil success:^(AFHTTPRequestOperation *opretion, id responseObject){
        ...
        myScrollView.backgroundColor = [UIColor clearColor];
        [weakSelf.view addSubview:myScrollView];
...
//        NSArray *arrauy=[dataDict objectForKey:@"thumbnail_images"];
        ...
        likeView = [[MsLikeView alloc]initWithFrame:FRAME(0, CGRectGetMaxY(myLabel.frame) - 20 , WIDTH, 550)];
        likeView.selectLikeBook = ^(NSDictionary *dataDict) {
            
            ArticleWEBViewController *webPageVC=[[ArticleWEBViewController alloc]init];
            webPageVC.barIDS=100;
            webPageVC.pushID=100;
            webPageVC.listID=[NSString stringWithFormat:@"%@",[dataDict objectForKey:@"id"]];
            webPageVC.webURL=[NSString stringWithFormat:@"%@",[dataDict objectForKey:@"url"]];
            [weakSelf.navigationController pushViewController:webPageVC animated:YES];
        };
        [myScrollView addSubview:likeView];
        
        myScrollView.contentSize=CGSizeMake(WIDTH, size.height+10+titleSize.height+(WIDTH-20)*0.75+30 + 560);
        
        [self requestWithCategories:categories];
...
-(void)requestWithCategories:(NSArray *)categories{

    NSString *urlStr = [NSString stringWithFormat:@"http://bolohr.com/?json=get_category_posts&count=5&order=DESC&id=%@&include=id,title,modified,url,thumbnail,custom_fields&page=1",categories[0][@"id"]];
    AFHTTPRequestOperationManager *mymanager = [AFHTTPRequestOperationManager manager];
    
    [mymanager GET:urlStr parameters:nil success:^(AFHTTPRequestOperation *opretion, id responseObject){
        
        NSLog(@"数据%@",responseObject);
        NSArray *array=[responseObject objectForKey:@"posts"];
        likeView.dataArr = array;
        [likeView reloadData];
    ...
    if (tableView==myTableView) {
        switch (indexPath.row) {
            case 0:
            {
                ...
                [UMSocialData defaultData].extConfig.sinaData.shareText = [NSString stringWithFormat:@"%@@菠萝HR_人事行政神器",webTitleLabel.text];//微博分享内容设置
                [UMSocialSnsService presentSnsIconSheetView:self appKey:YMAPPKEY shareText:webTitleLabel.text shareImage:[UIImage imageNamed:@"bolohr-logo512.png"] shareToSnsNames:[NSArray arrayWithObjects:UMShareToWechatSession,UMShareToWechatTimeline,UMShareToQQ,UMShareToQzone,UMShareToSina,nil] delegate:self];//UMShareToWechatTimeline,UMShareToQQ,UMShareToQzone,UMShareToSina
    ...
    if(fatherVc.loginYesOrNo!=YES)
    {
        ...
        switch (button.tag) {
            ...
            case 2:
            {
                ...
                [UMSocialQQHandler setQQWithAppId:@"1104934408" appKey:@"bRW2glhUCR6aJYIZ" url:webURL];
                [UMSocialSinaSSOHandler openNewSinaSSOWithAppKey:@"247547429" RedirectURL:webURL];
                [UMSocialSnsService presentSnsIconSheetView:self appKey:YMAPPKEY shareText:webTitleLabel.text shareImage:[UIImage imageNamed:@"bolohr-logo512.png"] shareToSnsNames:[NSArray arrayWithObjects:UMShareToWechatSession,UMShareToWechatTimeline,UMShareToQQ,UMShareToQzone,UMShareToSina,nil] delegate:self];
            }
                break;
            default:
                break;
