...
@interface Service_hallViewController ()
{
    NSArray *array;
    NSString *createPath;
}
...
@implementation Service_hallViewController
    ...
    for (int i=0; i<op_adArray.count; i++) {
        ...
        if (sqlite3_prepare_v2(simi, searchtUTF8, -1, &dbps, nil) != SQLITE_OK) {
            ...
            while (sqlite3_step(dbps) == SQLITE_ROW) {
                ...
                NSString *imageStr=[NSString stringWithFormat:@"apptools_%@_%@",t_idStr,update_timeStr];
...
-(void)preLoadLayout:(NSDictionary *)dict
{
    ...
    NSString *imageUrl=[NSString stringWithFormat:@"%@",[dict objectForKey:@"logo"]];
    NSString *t_idStr=[NSString stringWithFormat:@"%@",[dict objectForKey:@"t_id"]];
