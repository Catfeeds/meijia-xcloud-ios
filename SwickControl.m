    ...
    [CATransaction setCompletionBlock:^{
        ...
        if (_swickID==1) {
            if (_ID==1) {
                ...
                if (self.noId==0) {
                    if ([_switchStr isEqual:@"YES"]) {
                        
                    }else{
                        UIAlertView *tsView=[[UIAlertView alloc]initWithTitle:@"提醒" message:@"只有选择了秘书处理才可以选择订机票哦！" delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
                        [tsView show];
                    }
