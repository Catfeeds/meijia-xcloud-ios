...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= __IPHONE_10_0 // Xcode 8
...
//            _adView.backgroundColor=[UIColor redColor];
            ...
            //添加定时器
            //计时器
            timer = [NSTimer scheduledTimerWithTimeInterval:1.0f target:self selector:@selector(setUpValueForTimerLab) userInfo:nil repeats:YES];
            //添加跳过按钮
          UIButton *  disMissBtn = [[UIButton alloc]initWithFrame:FRAME(WIDTH - 83, 30, 68, 28)];
            [disMissBtn addTarget:self action:@selector(showWord) forControlEvents:(UIControlEventTouchUpInside)];
            [disMissBtn setBackgroundImage:[self imageWithColor:[UIColor blackColor]] forState:UIControlStateNormal];
            
            [splashView addSubview:disMissBtn];
            //添加按钮的文字
            disMissLabel = [[UILabel alloc]initWithFrame:FRAME(0, 0, 68, 28)];
            disMissLabel.font = [UIFont systemFontOfSize:18];
            disMissLabel.textAlignment = NSTextAlignmentCenter;
            disMissLabel.textColor = [UIColor colorForHex:@"ffffff"];
            NSMutableAttributedString *AttributedStr = [[NSMutableAttributedString alloc]initWithString:@"跳过 3"];
            ...
            disMissLabel.attributedText = AttributedStr;
            disMissBtn.layer.cornerRadius = 15;
            disMissBtn.layer.masksToBounds = YES;
            disMissBtn.alpha = 0.45;
            [disMissBtn addSubview:disMissLabel];
            
            
            [self performSelector:@selector(showWord) withObject:nil afterDelay:4.0f];
...
- (void)setUpValueForTimerLab {
    ...
    NSMutableAttributedString *AttributedStr = [[NSMutableAttributedString alloc]initWithString:[NSString stringWithFormat:@"跳过 %ld",timerCount]];
    [AttributedStr addAttribute:NSForegroundColorAttributeName
...
//    [[NSNotificationCenter defaultCenter] postNotificationName:@"IMGTAPP" object:nil];
    ...
    webPageVC.webURL=[NSString stringWithFormat:@"http://bolohr.com/a/load/"];
