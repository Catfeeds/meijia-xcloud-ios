...
@interface VideoPaymentViewController ()<BUYDELEGATE>
{
    //    页面布局
...
@implementation VideoPaymentViewController
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    double  mone=[self.moneyStr doubleValue];
    
    double actual=[self.actualStr doubleValue];
    double actMone=mone-actual;
    
    
    nowmoneyStr=[NSString stringWithFormat:@"%.2f",actMone];
    _buyview.selfmoney = self.moneyStr;
   
  
    //    实际支付
    _buyview.actualString=nowmoneyStr;
    //优惠
    _buyview.volumeString=[NSString stringWithFormat:@"为您节省%.2f元",actual];
...
-(void)viewLayout
{
    ...
    if ([_addssID intValue]==0) {
        _buyview = [[PayMentView alloc]initWithFrame:FRAME(0, 0, _WIDTH, _HEIGHT-64) num:3 addid:[_addssID intValue]];
    }else{
    ...
    _buyview.zhanghu = [NSString stringWithFormat:@"%@", _manager.telephone];
        ...
        _buyview.jine = [NSString stringWithFormat:@"%@",self.buyString];
    
    _buyview.delegate = self;
     double  mone=[self.moneyStr doubleValue];
    _buyview.fanxian =[NSString stringWithFormat:@"%.2f",mone];
;
            ...
            if (isAli) {
                ...
                if (moile==nil||moile==NULL||[moile isEqualToString:@""]) {
                    ...
                    if (res1 || res2 || res3 || res4) {
                        ...
                        if (addresIDS==0) {
                            if ([self.actualStr isEqualToString:@"0"]) {
